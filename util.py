...
def get_mean_std_per_batch(image_dir, df, H=320, W=320):
    sample_data = []
    for img in df.sample(100)["image_path"].values:
...
def get_roc_curve(labels, predicted_vals, generator):
    ...
    for i in range(len(labels)):
        try:
            ...
            auc_roc = roc_auc_score(gt, pred)
            auc_roc_vals.append(auc_roc)
            fpr_rf, tpr_rf, threshold = roc_curve(gt, pred)
            t = find_best_threshold(threshold, fpr_rf, tpr_rf)
            thresholds[labels[i]] = t
    ...
    plt.show()
    return auc_roc_vals,thresholds